...
/*Here we'll implement the logic of the 22 opcodes, initialize the instructions array, and the instruction's function pointers array
*Also containing the implementation of dealing with interrupts (used by main) and the logic of promoting counters. 
*/
...
void initInstructionArray(FILE* imemInFile) {
	...
	while (fgets(currentInstructionLine, LINE_LEN, imemInFile) != NULL) {
		...
		if (instructionArray[localPC].rd == 1 || instructionArray[localPC].rs == 1 || instructionArray[localPC].rt == 1) {
			...
			if (fgets(currentInstructionImm, LINE_LEN, imemInFile)) {		/*add 3 zeros from right, in order for strtoul to recognize negatives*/
				...
				instructionArray[localPC].immediate = tempImmediate >> 12;		/*shift back the 3 zeros added*/
			}
			localPC++;
		}
		localPC++;
...
/*Take the last 9 bits rd's content, set the jumpFlag to "Jumped"*/
...
/*Set the values of the instruction functions pointers in the array*/
...
int checkForIrq0() {
	...
	if (IORegisters[TIMERCURRENT].myValue > IORegisters[TIMERMAX].myValue) {	/*Triggered while instruction with immediate executed*/
		IORegisters[TIMERCURRENT].myValue = 1;//IORegisters[TIMERCURRENT].myValue - IORegisters[TIMERMAX].myValue;
		return 1;
	}
	return 0;
}
...
int checkForIrq2(Inst* prevInstruction) {
	...
	if (prevInstruction->isType2) {
		if (irq2values[0] == clockCycle - 1 || irq2values[0] == clockCycle - 2) {
			return 1;
		}
	}
	else {
		if (irq2values[0] == clockCycle - 1) {
			return 1;
		}
	}
	return 0;
...
/*Implement the logic to test if an interrupt should be executed*/
void checkInterrupts(Inst* prevInstruction) {
	...
	if (irq0Flag && IORegisters[TIMERENABLE].myValue) {
...
/*Promote timer's clock if needed*/
void promoteTimer() {
...
/*Promote disk's clock if needed*/
