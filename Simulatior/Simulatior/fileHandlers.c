...
/*Read data from dmemin file to dmem array*/
void readDMemIn() {
...
/*Build a 20 bit instruction code according to current PC*/
...
/*Write one line of trace to trace file according to current instruction, and assignment instructions*/
void writeTraceOutput(unsigned int opcode) {
	...
	for (i = 0; i < NUM_OF_REGISTERS; i++) {
		if (i != 1) {
			...
			if (instructionArray[PC].isType2) {
...
/*Write one line of HW register trace to trace file according to current instrction, assignment instructions*/
...
/*Write the data from dmem array to the dmemout file*/
void writeDMemOut() {
	int i;
...
/*Write the registers data to the regout file*/
void writeRegOut() {
	int i;
...
/*Write the monitor data from the monitor array the monitor file*/
...
/*Write the monitor data from the monitor array the monitor YUV file*/
...
/*Write the data from disk array array to the disk file*/
void writeDiskOut() {
...
/*Write the counter to the cycles file*/
void writeCycles() {
	fprintf(fcycles, "%d\n", clockCycle);
	fprintf(fcycles, "%d\n", instructionsCount);
}
